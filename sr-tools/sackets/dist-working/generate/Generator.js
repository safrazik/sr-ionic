...
function generateModel(modelName, modelKey, modelMeta, modelsMeta) {
    ...
    var tmp = { ModelName: modelName, ModelKey: modelKey, imports: '' };
    ...
    for (var propName in modelMeta) {
        ...
        if (propName == 'toString') {
            ...
            tmp.ModelToString = 'toString(){' + addLine(1) + addTab(2) + 'return `' + propMeta + "`.replace(/undefined/g, '').replace(/null/g, '').replace(/false/g, '');" /*END*/ + addLine(1) + addTab(1) + '}';
            continue;
        }
        ...
        var popFields = function (propName, propMeta, dataType, related) {
